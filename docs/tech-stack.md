...
- **2026-05-28** — **Decoupled the simulation from `requestAnimationFrame`**: the sim now runs on a fixed-timestep `setInterval` (so it keeps advancing when the tab is backgrounded — rAF pauses while hidden), and rAF drives only rendering + camera pan. This also makes tick rate independent of display refresh. (`app/play/Match.tsx`)
- **2026-05-28 (fix)** — Dropped the `next-themes` `ThemeProvider`; dark mode is now pinned via `class="dark"` on `<html>` and `<Toaster theme="dark" />`. Reason: next-themes injects a theme `<script>` that React 19 rejects on client-side navigation ("Encountered a script tag while rendering React component"), spamming the console when navigating Home → /play. `next-themes` stays installed only because the shadcn Sonner component imports `useTheme` (it returns safe defaults without a provider).
- **2026-05-30 (sprite/asset layer)** — Added `game/render/sprites.ts` — a **browser-only** art layer (kept out of `game/sim`, which stays engine-agnostic). It lazily loads + caches `HTMLImageElement`s from the **Kenney CC0 packs** under `public/assets/kenney/`, maps entities → sprites (per-player color variant), and exposes rotated-draw, a flashlight-cone draw, and CSS-cursor helpers. The canvas renderer calls these with a `ready()` guard and **falls back to the hand-drawn vector icons** until images decode, so SSR/first paint never breaks (no `Image` use at module load; preload runs in the match `useEffect`). Static assets are served straight from `public/` by Next.
