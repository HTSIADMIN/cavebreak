...
- **2026-05-28 (map system + multiple maps)** — Extracted a **map-descriptor system** into `game/sim/maps.ts`; `createInitialState(setup)` now consumes a `MapDef` and spawns 2–4 seats. All maps share the 64×64 grid (so renderer/camera/visibility sizing stays fixed) but vary their **silhouette** (a `shapeMask` paints unmineable `BOUNDARY` for non-square shapes), **start layout** (2–4 pockets, ordered for fair spread so taking the first N seats stays well-separated), and **features** (resource layout + carved corridor style). Shipped three:
...
- **2026-05-30 (sealed start + randomized resources)** — Per owner direction, **maps no longer pre-carve any corridors** — every map's `carve` is now a no-op, so you start sealed in your pocket and must mine your own way to everything (the hourglass wall/gap and circular rim still come from `shapeMask`; the gap is diggable `ROCK`). **Resource layout is now randomized & seeded per match** (`randomResources` in `maps.ts`): each active start gets one guaranteed near field (a 5–9 tile dig) so nobody is starved, then ~16 mineral/gas clusters (≈22% golden) scatter at random valid `ROCK` tiles, spread apart and off the bases. The per-match seed is randomized in `mapgen.ts` (`Math.random`) unless one is pinned (multiplayer/tests). Removed `carveLine`/`windingCorridor`/`spokesToCenter`/`mirror`. AI still macros on sealed maps via its existing offensive digging — just slower (it digs to resources and to the enemy), which is the intended feel.
