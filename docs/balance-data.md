...
- **2026-05-30 (construction + art)** — Added **`MAX_BUILDERS` 4** (cooperative, tied-up construction — see the Cave-Specific table + [buildings.md](./buildings.md)). No SC2 stat changes. **Art:** units/buildings now render from the **Kenney CC0 packs** in `public/assets/kenney/` (mapping + cursors + the flashlight cone live in `game/render/sprites.ts`, not here, since they're cosmetic — no balance impact). The Tiny RPG pack in `public/assets/tiny-rpg/` is intentionally unused for now.
- **2026-05-30 (renames + sealed/random maps)** — **Display names** changed (stat values unchanged): Worker→**Miner**, Zealot→**Brawler**, Stalker→**Gunner**; Nexus→**Base**, Pylon→**Generator**, Gateway→**Barracks**, Cybernetics Core→**Tech Lab**, Photon Cannon→**Turret** (Forge kept). Internal type IDs (`nexus`, `gateway`, …) are unchanged, so this file's tables still key by them. **Maps** are now sealed (no carved corridors) with **randomized, seeded resource placement** per match — see [map-terrain.md](./map-terrain.md).
