...
- **2026-05-30 (unit collision)** — Added soft unit-vs-unit separation + hard walls/structures (`resolveCollisions` in `world.ts`, one relaxation pass/tick after movement). Overlapping units push apart (half each); the push is **clamped per-axis to walkable floor** so units slide along — never phase through — rock or building footprints. Radius is per-state (`COLLISION_RADIUS` in `constants.ts`): **Miner 0.12** (crews pass through each other while mining/gathering), **combat 0.38** (idle/moving spacing), **fighting 0.25** (attacking/attack-moving shrink to ~half a tile so they can slip past in a 1-wide tunnel). Melee is unaffected (it uses tile adjacency, well outside these radii).
