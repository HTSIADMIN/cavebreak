...
- **2026-05-30 (flashlight visual)** — Added a **cosmetic** flashlight layer (no gameplay change — actual reveal stays the radial LOS above). For each local unit the renderer projects a light-mask **cone** (`game/render/sprites.ts` → `drawCone`, the Kenney `cone_b` mask) from the unit's position along its `facing`, composited additively over the lit ground. Gives the "fog-of-war flashlight" feel the owner asked for; it brightens what's already visible rather than gating sight.
- **2026-05-30 (flashlight removed)** — Removed the flashlight cones per owner feedback (too bright; muddied the fog). The renderer is back to the clean 3-state look: `vis 2` lit, `vis 1` dimmed (`rgba(0,0,0,0.5)`) in the terrain pass, `vis 0` black — units **and** buildings reveal their current surroundings, everything else stays dark. `drawCone`/`coneImage` remain in `sprites.ts` (unused) in case we revisit it; the cone asset is no longer preloaded.
